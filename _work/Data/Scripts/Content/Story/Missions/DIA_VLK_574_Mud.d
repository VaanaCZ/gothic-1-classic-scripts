...
{
	AI_Output (self, other,"DIA_Mud_FirstEXIT_07_00"); //Ehi, amico! Sei nuovo? Non ti ho mai visto prima.
	...
	Info_AddChoice	 (DIA_Mud_FirstEXIT,"Perché no? Andiamo!",DIA_Mud_FirstEXIT_Komm);
};

func void DIA_Mud_FirstEXIT_Verpiss()
{
	AI_Output (other, self,"DIA_Mud_FirstEXIT_Verpiss_15_00"); //Sparisci!
	AI_Output (self, other,"DIA_Mud_FirstEXIT_Verpiss_07_01"); //Giornata storta, eh? Farò meglio ad accompagnarti...
	Info_ClearChoices(DIA_Mud_FirstEXIT);
	AI_StopProcessInfos	( self );
...
{
	AI_Output (other, self,"DIA_Mud_FirstEXIT_Komm_15_00"); //Perché no? Andiamo!
...
INSTANCE DIA_Mud_ShutUp (C_INFO)
{
	...
	permanent	= 1;
	description	= "Potresti stare ZITTO un minuto?";
};                       
...
{
	AI_Output (other, self,"DIA_Mud_ShutUp_15_00"); //Potresti stare ZITTO un minuto?
	AI_Output (self, other,"DIA_Mud_ShutUp_07_01"); //Certo.
...
FUNC VOID DIA_Mud_GetLost_Info()
{
	...
	AI_Output (self, other,"DIA_Mud_GetLost_07_01"); //Vuoi restare solo per un po'? Capisco... Ti seguirò da lontano...
};
...
{
	AI_Output (self, other,"DIA_Mud_GetLost_07_00"); //Ehi, amico! Mi hai colpito! Perché mai?
...
{
	AI_Output (self, other,"DIA_Mud_Nerve_0_07_00"); //Dove stiamo andando? Oppure è una sorpresa? Mi piacciono le sorprese!
...
{
	AI_Output (self, other,"DIA_Mud_Nerve_1_07_00"); //Non preoccuparti per me: ti sarò sempre accanto.
	Npc_SetRefuseTalk(self, NerveSec);
	Mud_Nerve = 2;
...
{
	AI_Output (self, other,"DIA_Mud_Nerve_2_07_00"); //Sai dove stiamo andando?
	Npc_SetRefuseTalk(self, NerveSec);
	Mud_Nerve = 3;
	AI_StopProcessInfos	( self );
};
...
{
	AI_Output (self, other,"DIA_Mud_Nerve_3_07_00"); //Stai cercando un posto tranquillo? Dove possiamo parlare un po’?
	Npc_SetRefuseTalk(self, NerveSec);
	Mud_Nerve = 4;
	AI_StopProcessInfos	( self );
};
...
{
	AI_Output (self, other,"DIA_Mud_Nerve_4_07_00"); //Non è che stiamo camminando in cerchio? Cioè, non so dove vuoi andare...
	Npc_SetRefuseTalk(self, NerveSec);
	Mud_Nerve = 5;
	AI_StopProcessInfos	( self );
};
...
{
	AI_Output (self, other,"DIA_Mud_Nerve_5_07_00"); //Divertente. Grazie per avermi portato con te. Potremo farlo più spesso, abbiamo un sacco di tempo oggi.
	Npc_SetRefuseTalk(self, NerveSec);
	Mud_Nerve = 6;
	AI_StopProcessInfos	( self );
};
...
{
	AI_Output (self, other,"DIA_Mud_Nerve_6_07_00"); //Mi sono già affezionato a te. Ci siamo divertiti un sacco, insieme, vero? Dove hai intenzione di dormire? Puoi stare a casa mia, non c'è problema.
	Npc_SetRefuseTalk(self, NerveSec);
	Mud_Nerve = 7;
	AI_StopProcessInfos	( self );
};
...
{
	AI_Output (self, other,"DIA_Mud_Nerve_7_07_00"); //Perché ti hanno sbattuto quaggiù? Ti piacciono gli animali? C'entrano qualcosa? Sono loro il motivo per cui sono qui.
	Npc_SetRefuseTalk(self, NerveSec);
	Mud_Nerve = 8;
	AI_StopProcessInfos	( self );
};
...
{
	AI_Output (self, other,"DIA_Mud_Nerve_8_07_00"); //Alcune persone cercano sempre di picchiarmi. Gente orribile di Campo Nuovo, dei veri tagliagole. La prossima volta, dirò loro che d'ora in poi dovranno vedersela con te. Questo li spaventerà molto.
	Npc_SetRefuseTalk(self, NerveSec);
	Mud_Nerve = 9;
	AI_StopProcessInfos	( self );
};
...
{
	AI_Output (self, other,"DIA_Mud_Nerve_9_07_00"); //Quei bastardi di Campo Nuovo rimarranno di sasso, quando dirò loro che ho un protettore. Se la faranno addosso. Sarà divertente. Di solito fanno fuori la gente in fretta, ma stavolta ci penseranno due volte.
	Npc_SetRefuseTalk(self, NerveSec);
	Mud_Nerve = 10;
	AI_StopProcessInfos	( self );
};
...
{
	AI_Output (self, other,"DIA_Mud_Nerve_10_07_00"); //Sai una cosa? Ti devo un favore. Metterò una buona parola per te, così forse ti accetteranno nel campo. Così potremo incontrarci ogni giorno!
	Npc_SetRefuseTalk(self, NerveSec);
	Mud_Nerve = 11;
	AI_StopProcessInfos	( self );
};
...
{
	AI_Output (self, other,"DIA_Mud_Nerve_11_07_00"); //Certa gente pensa che io sia tutto scemo. Ma uno come te non andrebbe mai in giro con uno scemo. La gente ne parlerà. Così finalmente capiranno che si sbagliavano su di me.
	Npc_SetRefuseTalk(self, NerveSec);
	Mud_Nerve = 12;
	AI_StopProcessInfos	( self );
};
...
{
	AI_Output (self, other,"DIA_Mud_Nerve_12_07_00"); //Sei il mio migliore amico. Tutti gli altri mi evitano. Ti sono davvero grato.
	Npc_SetRefuseTalk(self, NerveSec);
	Mud_Nerve = 13;
	AI_StopProcessInfos	( self );
};
...
{
	AI_Output (self, other,"DIA_Mud_Nerve_13_07_00"); //Senti, mi sto davvero stancando di correre in cerchio. Perché non troviamo un bel posto e ci mettiamo comodi?
	Npc_SetRefuseTalk(self, NerveSec);
	Mud_Nerve = 14;
	AI_StopProcessInfos	( self );
};
...
{
	AI_Output (self, other,"DIA_Mud_Nerve_14_07_00"); //Sei come un fratello maggiore per me. Io non ne ho mai avuto uno. Nemmeno uno minore. I miei genitori non mi volevano. Deve essere stata dura, per loro.
	AI_Output (other, self,"DIA_Mud_Nerve_14_15_01"); //Non ho dubbi.
	Npc_SetRefuseTalk(self, NerveSec);
	Mud_Nerve = 15;
	AI_StopProcessInfos	( self );
};
...
{
	AI_Output (self, other,"DIA_Mud_Nerve_15_07_00"); //Da dove vieni? Sarai contento di aver incontrato uno che ti capisce, finalmente. Io sono di Khorinis. Ci sei mai stato? Beh, in fondo non fa differenza.
	Npc_SetRefuseTalk(self, NerveSec);
	Mud_Nerve = 16;
	AI_StopProcessInfos	( self );
};
...
{
	AI_Output (self, other,"DIA_Mud_Nerve_16_07_00"); //Non è che hai qualcosa da mangiare? Sono tuo ospite: dammi da mangiare e mi comporterò bene. Queste sono le buone maniere. Così mi dicevano i miei amici a Khorinis. Mi volevano davvero bene.
	Npc_SetRefuseTalk(self, NerveSec);
	Mud_Nerve = 17;
	AI_StopProcessInfos	( self );
};
...
{
	AI_Output (self, other,"DIA_Mud_Nerve_17_07_00"); //Pensi che qualcuno ci farà mai visita? Forse no: loro non riusciranno mai a uscire da qui, vero?
	Npc_SetRefuseTalk(self, NerveSec);
	Mud_Nerve = 18;
	AI_StopProcessInfos	( self );
};
...
{
	AI_Output (self, other,"DIA_Mud_Nerve_18_07_00"); //Ora che sei con me possiamo mostrare a un po' di persone dove devono andare. Quei bastardi mi hanno maltrattato per anni. Appena ne vedo uno, vado a dirgli due parole.
	Npc_SetRefuseTalk(self, NerveSec);
	Mud_Nerve = 19;
	AI_StopProcessInfos	( self );
};
...
{
	AI_Output (self, other,"DIA_Mud_Nerve_19_07_00"); //Noi due formiamo una squadra perfetta. Potremmo occupare l'intero Campo Vecchio. I baroni delle miniere non sarebbero un grosso problema per noi due. Penserò a un piano.
	Npc_SetRefuseTalk(self, NerveSec);
	Mud_Nerve = 0;
	AI_StopProcessInfos	( self );
};
