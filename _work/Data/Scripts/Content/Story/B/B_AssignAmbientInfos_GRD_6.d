...
{
	AI_Output(other,self,"Info_Grd_6_EinerVonEuchWerden_15_00"); //Nie przydałby się wam dodatkowy człowiek?
	AI_Output(self,other,"Info_Grd_6_EinerVonEuchWerden_06_01"); //Jesteś tu nowy, co? Jeśli chcesz szybko dołączyć do jakiegoś Obozu, spróbuj u tych świrów z sekty. Nie są zbyt wybredni, jeśli chodzi o nowych członków.
	AI_Output(self,other,"Info_Grd_6_EinerVonEuchWerden_06_02"); //Nie przyjmujemy byle kogo. No, chyba że chcesz pracować w Starej Kopalni!
};
...
{
	AI_Output(other,self,"Info_Grd_6_WichtigePersonen_15_00"); //Kto tu rządzi?
	AI_Output(self,other,"Info_Grd_6_WichtigePersonen_06_01"); //Thorus dba tu o względny porządek. Nad nim jest już tylko Gomez.
...
INSTANCE Info_Grd_6_DasLager(C_INFO)
{
	...
	permanent	= 1;
	description = "Jestem tu nowy. Co słychać?";
};                       
...
FUNC VOID Info_Grd_6_DasLager_Info()
{
	...
	AI_Output(self,other,"Info_Grd_6_DasLager_06_01"); //Właśnie widzę.
	AI_Output(other,self,"Info_Grd_6_DasLager_15_02"); //Co słychać?
	AI_Output(self,other,"Info_Grd_6_DasLager_06_03"); //Jak będziesz sprawiał kłopoty, dostaniesz w ucho.
	Info_ClearChoices(Info_Grd_6_DasLager);
	Info_AddChoice(Info_Grd_6_DasLager,"Rozumiem.", Info_Grd_6_DasLager_Verstehe);
	Info_AddChoice(Info_Grd_6_DasLager,"Co masz na myśli mówiąc 'kłopoty'?", Info_Grd_6_DasLager_WasIstAerger);
};
...
{
	AI_Output(other,self,"Info_Grd_6_DasLager_WasIstAerger_15_00"); //Co masz na myśli mówiąc "kłopoty"?
	AI_Output(self,other,"Info_Grd_6_DasLager_WasIstAerger_06_01"); //Większość Kopaczy płaci nam za ochronę.
	AI_Output(self,other,"Info_Grd_6_DasLager_WasIstAerger_06_02"); //Jak zaczepisz któregoś z nich, będziesz miał do czynienia z nami.
	AI_Output(self,other,"Info_Grd_6_DasLager_WasIstAerger_06_03"); //Jeśli złapię cię na grzebaniu w cudzych chatach...
	AI_Output(other,self,"Info_Grd_6_DasLager_WasIstAerger_15_04"); //Dobrze, dobrze. Zrozumiałem aluzję.
	Info_ClearChoices(Info_Grd_6_DasLager);
};
...
FUNC VOID Info_Grd_6_DieLage_Info()
{
	...
	AI_Output(self,other,"Info_Grd_6_DieLage_06_01"); //Szukasz kłopotów?
};
