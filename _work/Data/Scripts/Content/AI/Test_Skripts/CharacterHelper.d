...
func VOID CH_Guild_Info()
{
	...
	if ((hero.level >= 10))
	{    
	    Info_AddChoice		(CH_Guild, "Sцldner"       , CH_Mercenary);
	};
	if ((hero.level >= 16))
	{    
...
func void CH_Training_STR_Info()
{
	...
	Info_AddChoice		(CH_Training_STR, "ZURЬCK"	     							, CH_Training_STR_BACK);
	Info_AddChoice		(CH_Training_STR, "Stдrke (alle restlichen Lernpunkte)"		, CH_Strength_all);
	Info_AddChoice		(CH_Training_STR, "Stдrke (fьr beste Nahkampfwaffe)"		, CH_Strength_Weapon);
...
func void CH_Training_DEX_Info ()
{
	...
	Info_AddChoice		(CH_Training_DEX, "ZURЬCK"	     							, CH_Training_DEX_BACK);
	Info_AddChoice		(CH_Training_DEX, "Geschick (alle restlichen Lernpunkte)" 	, CH_Dexterity_all);
	Info_AddChoice		(CH_Training_DEX, "Geschick (fьr beste Fernkampfwaffe)"		, CH_Dexterity_Weapon);
	if	(hero.LP >= 20)
	{ 
