...
const int	AIV_FINDABLE					= 7; // MH: Fьr Info_FindNPC
const int 	AIV_PLUNDERED					= 8;	// wird gesetzt sobald der Kцrper geplьndert wurde und gelцscht, wenn der NSC ohnmдchtig/tot wird
const int	AIV_WARNTARGET					= 9;
const int	AIV_LASTHITBYRANGEDWEAPON		= 10; // SN: Merkt sich, ob der letzte erlittene Schaden durch eine Fernkampfwaffe/Magie verursacht wurde. Wird fьr einen intelligenten Abbruch des ZS_Attack benцtigt.
...
//-------- Durchgangswachen --------
const int	AIV_GUARDPASSAGE_STATUS			= 27;	// Enthдlt den Status einer Durchgangswache
const int		AIV_GPS_BEGIN			= 0;		// ...Anfang; SC noch nicht in der Nдhe
...
const int	AIV_LASTDISTTOWP				= 28;	// enthдlt die Distanz zu einem Referenzpunkt hinter der Wache, die bei der letzten Verwarnung ermittelt wurde
const int   AIV_PASSGATE					= 29;   // fьr Durchgangswachen 0 = Wache hдlt SC auf, 1 = Wache lдsst SC durch

const int 	AIV_DIDWAITTOCLEARROOM			= 30;	
const int	AIV_FOUNDPERSON					= 31;	// In einigen TAs wird mit Npc_PerceiveAll nach bestimmten Personen gesucht. Damit dies nicht stдndig gemacht werden muЯ, wird hier gemerkt, ob diese Person schon gefunden wurde

const int   AIV_LOCATION					= 32; // MH: fьr Aufenthaltsort - wichtig fьr Dialoge (kann LOC_OLDCAMP, LOC_NEWCAMP, etc. sein)
const int   AIV_INVINCIBLE					= 33; // MH: ist TRUE fьr beide Teilnehmer eines Dialogs (s. ZS_Talk)
...
const int	AIV_PARTYMEMBER					= 36;	//SN: (begleitet den Spieler - fьr XP-Vergabe)
...
const int	AIV_LASTTARGET					= 39;	//SN: wird in ZS_Attack benцtigt um das letzte Ziel zu merken
const int	AIV_TALKBEFOREATTACK			= 40;	//SN: ist dieses Flag gesetzt, so wird beim B_AssessEnemy auf eine Importantinfo abgefragt!
const int	AIV_DEALDAY						= 41;	//SN: wird fьr Ambient NSCs benutzt, um sich den Tag des letzten Deals zu merken (z.B.:Sumpfkraut fьr ORG oder Wasser fьr BAU)
...
//!!!! CONSTANTS.D, mьssen aber im nдchsten Release (v0.94) hier 
//!!!! aktiviert werden.
//---------------------------------------
...
**					    der Menschen-AI							   **
********************************************************************/
const int	HAI_DIST_MELEE					= 600;							// 1. Drunter wдhlen menschliche NSCs Nahkampf, drьber Fernkampf (B_SelectWeapon)
																			// 2. Innerhalb dieser Distanz fьhlen sich NSCs von gezogenen Waffen bedroht (B_AssessFighter)
const int	HAI_DIST_RANGED					= 3000;							// 1. Innerhalb dieser Distanz fьhlen sich NSCs von gezogenen Fernkampfwaffen/Kampfzaubern bedroht (B_AssessFighter)

const int	HAI_DIST_ABORT_MELEE 			= HAI_DIST_MELEE+400;			// NSC bricht einen Nahkampf ab, wenn die Distanz zum Ziel diesen Wert erreicht. '+X00' fungiert als Hysterese, damit nicht stдndig der Kampf beginnt und wieder endet	
const int	HAI_DIST_ABORT_RANGED 			= HAI_DIST_RANGED+500;			// NSC bricht einen Fernkampf ab, wenn die Distanz zum Ziel diesen Wert erreicht. '+X00' fungiert als Hysterese, damit nicht stдndig der Kampf beginnt und wieder endet	
...
const int	HAI_DIST_HELPATTACKEDCHARGES	= 2000;							// Wach-NSCs reagieren auf Angriffe auf beschьtzte NSCs innerhalb dieser Reichweite (gemessen am Opfer!)

const int	HAI_DIST_ASSESS_MONSTER			= 1000;							// Ab dieser Distanz zu einem herannahenden Monster reagiert der NSC darauf !
const int	HAI_DIST_ABORT_ASSESS_MONSTER	= HAI_DIST_ASSESS_MONSTER+200;	// Ab dieser Distanz beendet ein NSC die Abwartende Haltung gegenьber einem sich entfernenden Monster!
const int	HAI_DIST_ATTACK_MONSTER			= 1000;							// Ab dieser Distanz greift ein abwartender NSC ein herannahendes Monster an!

const int	HAI_DIST_CATCHTHIEF				= 150;							// Ab dieser Distanz ist es mцglich dem Nsc in die Tasche zu greifen
const int	HAI_DIST_ASSESSTHEFT			= 800;							// Auf diese Distanz stellen Nscґs einen Diebstahl fest
...
const int	HAI_DIST_CLEARROOM				= 700;							// innerhalb dieser Distanz reagieren nicht-Wachen auf das Betreten von unerlaubten Rдumen (PERC_ASSESSENTERROOM)
const int	HAI_DIST_HEARROOMINTRUDER		= 700;							// innerhalb dieser Distanz wird der SC beim Betreten von unerlaubten Rдumen auch entdeckt (gehцrt), wenn die Beobachter ihm den Rьcken zudrehen (aber schleichen hilft wieder!)
...
const int	HAI_DIST_FOLLOWPC				= 500;							// NSC im TA-Zustand ZS_FollowPC entfernt sich hцchstens bis zu dieser Distanz. Bei Ьberschreiten schlieЯt er wieder auf
...
const int	HAI_TIME_UNCONSCIOUS			= 20;							// Zeit in Sekunden, die der SC und NSCs bewuЯtlos bleiben
