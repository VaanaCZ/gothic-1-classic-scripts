...
// Fьge NSC in Welt ein. Wobei SPawnpoint entweder ein WP oder ein FP sein darf. Stirbt dieser NSC wird
...
// Hiermit wird das angegebene Item aus der Welt entfernt und gelцscht
...
// das angegebene Item wird aus dem Inventory des NSCs entfernt und gelцscht
func void	Npc_RemoveInvItems			(VAR C_NPC owner, VAR int itemInstance, VAR INT amount )	{ };
// das angegebene Anzahl des Multi-Items wird aus dem Inventory des NSCs entfernt und gelцscht
...
// Ziehe die angebene Rьstung dem NSC "owner" an, diese muss sich in seinem Inventory befinden.
// -------------------------------------------------------------------------------------------
...
// Цffnet ein Dokument mit dem Bild "picName" als Hintergrund
FUNC VOID 	Doc_Font					(VAR STRING fontName ) {};
// Benutzt den angegebenen Font fьr nachfolgende "Doc_Print"s.
FUNC VOID 	Doc_Print					(VAR STRING text ) {};
// Angegebener Text wird in das aktuelle (vorher mit Doc_Open geцffnete) Dokument geschrieben
...
// pixelx1,pixely1 : PixelKoordinaten linke obere Ecke // der Rest : alles nochmal fьr die rechte untere Ecke
...
// Hier kann ein Wert fьr den Zauberspruch gesetzt werden. Was dieser Wert bewirkt, haengt allein von der Nutzung im
...
// *******************************************************************************************
func int	Doc_Create		()
// erstellet ein Dokument und liefert ein Handle zurьck, daЯ fьr alle weiteren Doc_...-Befehle nцtig ist
...
// setzt fьr die Seite 'page' die Grafik-Datei 'pageimage'. Diese muЯ im TGA-Format vorliegen und 
// z.B. "aufgeschlagenesBuch_links.tga"  heiЯen
// wird bei 'page' -1 ьbergeben, so bezieht sich der Befehl auf ALLE Seiten des Dokuments
func void	Doc_SetFont		(var int handle, var int page, var string fontname) 	
// setzt den Font fьr die angegebene Seite 'page' oder fьr das ganze Dokument ('page':-1)
...
// setzt die Rдnder (gerechnet vom Rand der TGA-Datei, die in Doc_SetPage() gesetzt wird). Die Rдnder sind in Pixeln angegeben.
func void	Doc_PrintLine	(var int handle, var int page, var string text)
// Gibt Text fьr die Seite 'page' an. Der Text wird NICHT umgebrochen 
func void	Doc_PrintLines	(var int handle, var int page, var string text)
// Gibt Text fьr die Seite 'page' an. Der Text WIRD umgebrochen, falls er nicht auf die in Doc_SetPage() angegebene Grafik passt.
func void	Doc_Show	   	(var int handle)
 // nachdem das Dokument mit den obenstehenden Befehlen befьllt und formatiert wurde, wird es hiermit nun auf den Bildschirm geschrieben!            
