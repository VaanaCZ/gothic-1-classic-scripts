func void B_ReactToMemory ()
{
			...
			if ( Npc_GetPermAttitude ( self, other) == ATT_FRIENDLY || Npc_GetPermAttitude ( self, other) == ATT_NEUTRAL)
			{
				...
				//	Wichtige Gilden (GURU, EBR, MAGE) haben eigenes SVM
					...
					if ( murdernews_victim.guild == GIL_EBR)
					{
						...
						// JP: Neue Funktion, um erteilte News zu lцschen
				...
				//	Ein SVM fьr restl. Gilden Altes Lager
				...
				//	Ein SVM fьr restl. Gilden Neues Lager
	...
	if (defeat_news > 0)
	{
		...
		//JP: hier statt dem Victim und den news evtl. mit der AIV_Defeat arbeiten, vielleicht sogar zusдtzlich
		if (B_CompareNpcInstance ( other, defeat_news_offender))
		{
			...
			if (B_CompareNpcInstance ( self, defeat_news_victim))
			{
				...
				// JP: Warum wird hier nicht noch mal ьber stronger gegangen, was ist hier mit dem FuЯvolk kann es sich beschweren ( kann fьr sie der Fall eintreten) und soll
				// es sich beschweren ?
	...
	//JP: hier wurde auf eine Var abgefragt, die erst in dem Zweig gefьllt wird, grober Blцdsinn B_CompareInstance jetzt eine Ebene tiefer
	if (theft_news > 0 )
	{
