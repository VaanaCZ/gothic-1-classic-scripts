...
// Je hцher	die	ID,	desto wichtiger	ist	die	News ( Priorisierung ! )
...
// -> Einspeisen der News ins Gedдchtnis
// -> Reaktion auf News	( Attitьdenдnderung	)
// Fьr alle	Zeugen und News-Adressaten
...
// vict			:	Opfer
{
	...
	victimguild 		= Npc_GetTrueGuild ( vict); 			// Zuweisung der Gilde des Ьbergebenen Vict 
	offenderTrueGuild 	= Npc_GetTrueGuild ( offender); 		// Zuweisung der Gilde des Ьbergebenen Offender 
	witnessguild 		= Npc_GetTrueGuild ( witness); 			// Zuweisung der Gilde des Ьbergebenen witness 
	...
	if (newsid == NEWS_MURDER)
	{
		...
		Npc_MemoryEntry (witness,source,offender,newsid,vict ); // Und News merken hier sollte eigentlich Gilde als Opfer, es ist aber  
															 //	mцglich, in anderen Zustдnden als member von C_Npc auf die Gilde 
															 //	des Nsc zuzugreifen
	...
	else if (newsid == NEWS_DEFEAT)
	{
		...
		// JP: kein defeat mehr als Wiedergutmachung, aber wenn nur Temp Hostile wird die runtergesetzt, da man sonst nach dem Aufwachen direkt wieder niedergeknьppelt wird
