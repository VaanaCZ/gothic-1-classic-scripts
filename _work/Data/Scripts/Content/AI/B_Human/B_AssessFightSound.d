...
//	die jedesmal ausgelцst wird, wenn SC/NSC attackiert werden (Nah-,
//	Fern-, Magiekampf).
//	Macht im Grunde folgende Fallunterscheidung
//	1. 	Monster kдmft gegen Mensch
//		-> Monster bewerten in ZS_AssessMonster
//	2.	NSC ist Wache
//		-> greift ein, wenn ein Schьtzling angegriffen wird
//	3.	NSC ist eine Meister-Gilde
//		-> greift garnicht ein, ist aber wachsam
//	4.	NSC ist ein Normalbьrger
...
func void B_AssessFightSound ()
{
	...
	if (C_NpcIsMonster(other) && C_NpcIsHuman(victim) )
	{
		...
		if ((Npc_GetAttitude	(self, victim) == ATT_FRIENDLY) || (Npc_GetAttitude(self, victim) == ATT_NEUTRAL))
		{
			...
			/********************************************
			// Der  B wurde nicht abgebrochen, obwohl ein Monster beteiligt war, so daЯ sich die AI mit dem Monster als other weiter unten vertan hat
			/*************************************************/
			return;
		}
		else
	...
	//-------- WACHEN checken, ob einer ihrer Schьtzlinge angegriffen wird --------
	if (C_ChargeWasAttacked(self, victim, other))
	{ 
		PrintDebugNpc		(PD_ZS_CHECK,	"...NSC ist Wache und ein Schьtzling wurde attackiert!" );
		if (Npc_GetPermAttitude(victim,other) == ATT_FRIENDLY)		// HINWEIS: Bedingung fьr 'friendly fire' aus ZS_ReactToDamage!!!
		{
