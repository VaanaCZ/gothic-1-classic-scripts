...
{	
    ...
    AI_SetWalkmode 		(self,	NPC_RUN);				// Walkmode fьr den Zustand
};
...
func int ZS_Attack_Loop()
{
	...
	//-------- Wenn Gegner BewuЯtlos oder Tod... --------
	if (C_NpcIsDown(other) || !Hlp_IsValidNpc(other))
	{
		...
		if (Npc_GetNextTarget(self))			// Vorsicht: lцscht 'other', wenn kein neues Ziel gefunden
		{
	...
	if (Npc_GetStateTime(self) > 2)
	{
		...
		&&	(!Npc_IsInFightMode(other,	FMODE_FAR)	&&	!Npc_IsInFightMode(other,	FMODE_MAGIC))	)
		{
			...
			if ( (Npc_GetDistToNpc(self, other)>HAI_DIST_ABORT_MELEE) && (self.aivar[AIV_LASTHITBYRANGEDWEAPON]==FALSE) )
			{
				...
				if (C_NpcIsHuman(other))
				{
					...
					if (Npc_GetPermAttitude (self, other) != ATT_HOSTILE) //MH: sonst kommt SVM pьntig!
					{
		...
		var int countPursuitCycles;									// 2. Zдhler neben StateTime fьr Verfolgungsabbruch nach  Zeit
		...
		PrintGlobals					(PD_ZS_DETAIL);
		Npc_ClearAIQueue				(self);						// falls sich FAI-Moves schneller ansammeln, als sie abgearbeitet werden kцnnen.
		B_SelectWeapon					(self,	other);	
...
{	
	...
	other = Hlp_GetNpc(self.aivar[AIV_LASTTARGET]);			//SN: 'other' wird durch ein erfolgloses Npc_GetNextTarget() manchmal gelцscht!

	PrintGlobals			(PD_ZS_CHECK);
	
	/* ---- Ist der Gegner "nur" bewuЯtlos...  ---- */
...
//------------------------------------------------------------------------------------------------------
// Die letzt Chance fьr einen Abbruch des Kampfes
//
// Bedingung:
...
//------------------------------------------------------------------------------------------------------
// Der Spieler klaut wдhrend der Nsc Kдmpft
//
// Bedingung:
