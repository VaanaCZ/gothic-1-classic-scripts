...
// 										KONSTANTEN
// *************************************************************************************
// SN: Sind aus Grьnden der nцtigen Compile-Reihenfolge in AI_INTERN\AI_MM_CONSTANTS.D gewandert
...
func void B_MM_AssessEnemy()
{
	...
	if (	(Npc_GetDistToNpc(self, other) < self.aivar[AIV_MM_PercRange])
	&&		Npc_CanSeeNpcFreeLOS(self,other)									)	//SN 08.01.01: Vermeidung von Entdeckung des Spieler durch Wдnde hindurch
	{
		if (C_PreyToPredator(other,self)) 	//Jдger greift Beute immer direkt an
		{
			Npc_SetTarget 	(self,other);
			Npc_ClearAIQueue(self);
			AI_StartState 	(self, ZS_MM_Attack, 0, "");
		}
		else if (C_PreyToPredator(self,other))	// Beute flieht immer vor Jдger
		{
			Npc_SetTarget 	(self,other);
			Npc_ClearAIQueue(self);
		...
		else if (Wld_GetGuildAttitude(self.guild,other.guild) != ATT_HOSTILE)	
		{
			...
			//	-> vermeidet z.B. daЯ ein SC-Snapper von anderen Snappern attackiert wird
			//	-> wird spдter durch Aufruf von B_InitMonsterAttitudes() nach der Verwandlung dauerhaft und sauber gelцst! 
			return;	
		}
		else
		{	
...
func void B_MM_ReactToDamage ()
{
	...
	if (C_PreyToPredator(self,other))
	{
		...
		Npc_SetTarget 	(self, other);	//fьr AI_Flee
		Npc_ClearAIQueue(self);
		Npc_PercDisable	(self,	PERC_ASSESSENEMY);		// SN:da diese Wahrnehmung sonst die Ausfьhrung des folgenden AI_StartState verhindert!
		AI_StartState 	(self, ZS_MM_Flee, 0, "");	
	}
	else //Kein Jдger
	{
...
func int ZS_MM_Attack_Loop ()
{
	...
	else // GetTarget = FALSE
	{
		...
		{
			Npc_PerceiveAll		(self);		// nцtig, da Npc_GetNextTarget() auf der Liste der zuletzt Wahrgenommenen VOBs beruht
...
{
    // MH: BEACHTEN: Wenn Warns andere NSCs dureinanderbringen, mьssen diese die Sender FILTERN!
    
    PrintDebugNpc		(PD_MST_FRAME, "B_MM_AssessWarn");
		
	// ---------------- Beute von Jдger gewarnt -------------------
	if (C_PreyToPredator(self,other))
	{
		Npc_SetTarget	(self, other); 				//fьr AI_Flee
		Npc_ClearAIQueue(self);
	...
	// ---------------- PACKHUNTER greift an ODER ruft Verstдrkung -----------------
	if ( (Wld_GetGuildAttitude(self.guild,other.guild)==ATT_FRIENDLY) && (self.aivar[AIV_MM_Behaviour]==PACKHUNTER) )	
	{
		...
		{
			AI_SetWalkmode	(self, NPC_RUN);
			AI_GotoNpc 		(self, other);		// Gehe zu Jдger-Freund
...
func void B_MM_AssessQuietSound_Sleep()
{
	...
	*/	
	...
	if (Npc_GetDistToNpc(self, other) <= self.aivar[AIV_MM_DrohRange])	//Drohrange > INTERMEDIAT bringt natьrlich nix wegen QuietSound-Distanz
	{
