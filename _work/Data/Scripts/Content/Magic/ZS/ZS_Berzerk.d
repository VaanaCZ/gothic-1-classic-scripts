...
func int ZS_Berzerk_Loop()
{
	...
	Npc_GetTarget			(self);	
	if (Hlp_IsValidNpc	(other)		// Ziel technisch gьltig?
	&&	!C_NpcIsDown	(other) 	// Ziel am Leben?
	&&	!Npc_IsPlayer	(other)	)	// Ziel ein NSC (beim ersten Durchlaufen kцnnte es der Spieler sein!)	
	{																							
		//-------- Ziel angreifen ! --------
	...
	else
	{
		...
		if ( Wld_DetectNpcEx( self, -1, NOFUNC, -1, FALSE ) )	// ...und wдhlt irgendeinen davon aus (aber ignoriert den SC!)
		{
		...
		else
		{
			...
			AI_GotoWP 		(self,	Npc_GetNearestWP(self));	// zum nдchsten...	
			B_SayOverlay	(self,	NULL,	"$Berzerk");
			AI_PlayAni		(self,	"T_SEARCH");
			AI_GotoWP 		(self,	Npc_GetNextWP 	(self));	// dann zum zweitnдchsten Waypoint (beide Messungen bei t=0!)
			B_SayOverlay	(self,	NULL,	"$Berzerk");
			AI_PlayAni		(self,	"T_SEARCH");
		};
	};	
